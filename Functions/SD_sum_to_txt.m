function DATA = SD_sum_to_txt(DATA_in, campaign, flight, tstep_sec, a, b, bins)
...
row9 = ['3rd Column: Ntot, Total concentration [L-1]'];
row10 = ['4th Column: dNtot, Uncertainty in total concentration [L-1]'];
row11 = ['5th Column onwards: dN/dlogDp, Concentration in size bins [L-1]'];
...
SD_data = num2str(DATA_in(2:end,2:end));
timestr = datestr(DATA_in(2:end,1),'yyyy-mm-dd HH:MM:SS');

for i=1:size(DATA_in,1)-1
    DATA_out{18+i,1} = timestr(i,:);
    DATA_out{18+i,2} = SD_data(i,:);
end
